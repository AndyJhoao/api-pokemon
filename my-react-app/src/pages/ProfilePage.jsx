    ...
    <div className="flex-1 flex items-center justify-center px-4">
      <div className="bg-white border-2 border-blue-200 dark:bg-white/5 dark:border-white/10 backdrop-blur-md rounded-2xl shadow-2xl p-8 w-full max-w-sm">
        <div className="flex flex-col items-center mb-4">
          <div className="w-24 h-24 rounded-full bg-blue-50 border-2 border-blue-200 dark:bg-white/10 dark:border-white/20 flex items-center justify-center overflow-hidden mb-2">
            {avatarSrc ? (
              <img
                src={avatarSrc}
                alt="Profile Pokemon"
              ...
              <svg xmlns="http://www.w3.org/2000/svg" className="w-10 h-10 text-blue-600 dark:text-gray-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}>
                <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" />
                <circle cx="12" cy="7" r="4" />
              </svg>
            )}
          </div>
          <h2 className="text-2xl font-bold text-gray-800 dark:text-white">Trainer Profile</h2>
          <p className="text-gray-600 dark:text-gray-500 text-sm">{user.email}</p>
        </div>

        {saved && (
          <div className="bg-green-100 border-2 border-green-300 dark:bg-green-500/10 dark:border-green-500/30 rounded-lg px-4 py-2 text-green-700 dark:text-green-400 text-sm text-center mb-4">
            Profile updated!
          </div>
        )}
        {error && (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="text-xs text-gray-700 dark:text-gray-500 uppercase tracking-widest mb-1 block font-semibold">Display Name</label>
            <input
              type="text"
              value={displayName}
          ...
          <div>
            <label className="text-xs text-gray-700 dark:text-gray-500 uppercase tracking-widest mb-1 block font-semibold">Favorite Pokemon</label>
            <Autocomplete
          ...
          <div>
            <label className="text-xs text-gray-700 dark:text-gray-500 uppercase tracking-widest mb-1 block font-semibold">Profile Picture Pokemon</label>
            <Autocomplete
            ...
            />
            {spriteError && (
              <p className="text-red-600 dark:text-red-400 text-xs mt-1 font-medium">Pokemon not found</p>
            )}
          </div>
          <button
            type="submit"
